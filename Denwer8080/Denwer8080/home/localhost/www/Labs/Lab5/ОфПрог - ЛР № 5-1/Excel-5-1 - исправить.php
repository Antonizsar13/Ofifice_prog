...
<style type = "text/css">
       div { font-size: 18pt; font-family: tahoma;          
             color: navy; background-color: rgb(250,240,130); 
             margin: 5px; width: auto; padding: 5px; 
             text-align: center; }
       table { border: solid thin silver; background-color: rgb(134,251,140); }
       td { border: solid thin navy;
            text-align: center; font-size: 12pt;
            vertical-align: middle; font-family: tahoma;
            height: 50px; color: navy; }
       th { text-align: center;
            font-style: normal; font-size: 14pt;
            font-family: tahoma; height: 50px; color: navy;
            background-color: rgb(234,251,140); }
       ._td { border: solid thin red;
              text-align: center; font-size: 12pt;
              vertical-align: middle; font-family: tahoma;
              height: 40px; color: navy; background-color: rgb(255,198,226); }
       .__td { border: solid thin red;
               text-align: right; font-size: 12pt;
               vertical-align: middle; font-family: tahoma; padding-right: 10px;
               height: 30px; color: blue; background-color: rgb(255,198,226); }
    input[type="submit"] { 
          color: white; background-color: rgb(290,140,180);
          font-family: Arial; font-size: 15pt; border: 1px dotted navy; 
          border-radius: 10px; margin: 10px; 
          align-text: center; padding: 15px; }
    input[type="submit"]:hover { box-shadow: 0 12px 16px 0 rgba(0,0,0,0.24), 0 17px 50px 0 rgba(0,0,0,0.39); }
</style>						
</head>
<body>
<form method = 'post'>
  ...
  <tr bgcolor = "#88CC99">
    <td><input type = "submit" size ="40" name="form_select_button1" 
               value="Выдать документ Excel-1">
    </td>
  </tr>
  </table>
</form>	
...
function SelectButtonFun1()
 { 
   ...
   try 
   {
     ...
     if ( !$link ) { throw new Exception('error...'); exit(); }
   }
   catch (Exception $e)
   {
   ...
   if ( !$res ) 
   {
      echo '<div>Ошибка обращения к базе данных на сервере...</div>';
      die();
   }
   ...
   $style_4 = array('alignment' => array(
    ...
    $fill_array = array('alignment' => array(
                        'horizontal' => PHPExcel_STYLE_ALIGNMENT::HORIZONTAL_RIGHT,
                        'vertical' => PHPExcel_STYLE_ALIGNMENT::VERTICAL_CENTER ),
    ...
    $fill_cells_array = array('font'=>array( 'bold' => false,
                              'name' => 'Tahoma', 'size' => 12,
    ...
    $sheet->getStyle(CellAddress(3,3).':'.CellAddress(7,3))->getFont()->getColor()->
                applyFromArray(array('rgb' => '009966', 'bold'=> true));
    ...
    while ($row = mysqli_fetch_array($res, MYSQL_ASSOC))
       {
         ...
         if ( !isset($row['Su']) ) { $su = 0.0; } else { $su = $row['Su']; }
         if ($i == 1) // Самая первая по порядку строка обработки
            ...
            {
             if ($row['Otdel'] == $otdel) { $sum_otdel += $su; } //Если отдел тот же - суммируем зарплату
             else // иначе - выводим справа в таблице накопленную сумму по отделу:
                {
                  //Цветовые настройки для суммы зарплат по отделу:
                  $sheet->getStyle(CellAddress(7,$i+2))->getFont()->getColor()->
                               applyFromArray(array('rgb' => 'FF0000', 'bold' => false));

                  //Вывод суммы зарплат по отделу:                 
                  $sheet->setCellValueByColumnAndRow(7,$i+2,number_format($sum_otdel,2));
                  $otdel = $row['Otdel'];
         ...
         $sheet->getDefaultStyle()->getFont()->getColor()->
                      applyFromArray(array('rgb' => '000000'));
         ...
         $sheet->setCellValueByColumnAndRow(6,$i+3,number_format($su,2));
    ...
    $sheet->getStyle(CellAddress(3,$i+3))->getFont()->
                     applyFromArray(array('bold' => true));   
    ...
    $sheet->setCellValueByColumnAndRow(7,$i+3,number_format($sum_otdel_total,2));
 ...
 if ( isset($_POST['form_select_button1']) ) { SelectButtonFun1(); }
